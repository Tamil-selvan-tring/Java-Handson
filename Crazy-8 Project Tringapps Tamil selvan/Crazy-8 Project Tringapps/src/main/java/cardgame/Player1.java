...
public class Player1 implements PlayerStrategy {
	...
	public List<Card>player1_cards=new ArrayList<>();
	Card currentcard;
	public int score;
	Suit declared;
	...
	public Card playCard(boolean wildcard) {
		...
		if(wildcard)
		{
			...
			{
				player1_cards.remove(Wildcard);
				System.out.println("Player 1:"+ player1_cards.size());
				return Wildcard;
			}
			else if(Wildcard==null && playcard==null)
			{
				while(true && !Game.cardDeck.isEmpty())
				{
					...
					player1_cards.add(Game.cardDeck.get(0));
					Game.cardDeck.remove(0);
				}
			}
			System.out.println("Player 1"+ player1_cards.size());
